...
Received: from lists.ozlabs.org (lists.ozlabs.org [IPv6:2401:3900:2:1::3])
	by mail.lfdr.de (Postfix) with ESMTPS id 2ACD62C9C98
	for <lists+linux-aspeed@lfdr.de>; Tue,  1 Dec 2020 10:36:50 +0100 (CET)
Received: from bilbo.ozlabs.org (lists.ozlabs.org [IPv6:2401:3900:2:1::3])
	by lists.ozlabs.org (Postfix) with ESMTP id 4ClcSH2G5DzDqx5
	for <lists+linux-aspeed@lfdr.de>; Tue,  1 Dec 2020 20:36:47 +1100 (AEDT)
X-Original-To: linux-aspeed@lists.ozlabs.org
Delivered-To: linux-aspeed@lists.ozlabs.org
Authentication-Results: lists.ozlabs.org; spf=pass (sender SPF authorized)
 smtp.mailfrom=aspeedtech.com (client-ip=40.107.130.131;
...
Received: from APC01-HK2-obe.outbound.protection.outlook.com
 ...
 by lists.ozlabs.org (Postfix) with ESMTPS id 4ClcS70MyBzDqKq
 for <linux-aspeed@lists.ozlabs.org>; Tue,  1 Dec 2020 20:36:36 +1100 (AEDT)
...
Received: from PS1PR06MB2600.apcprd06.prod.outlook.com (2603:1096:803:4d::19)
 by PS1PR06MB2629.apcprd06.prod.outlook.com (2603:1096:803:4a::18)
...
To: Andrew Jeffery <andrew@aj.id.au>, Stefan M Schaeckeler
...
x-forefront-antispam-report: CIP:255.255.255.255; CTRY:; LANG:en; SCL:1; SRV:;
 ...
 SFS:(39840400004)(136003)(346002)(366004)(396003)(376002)(7696005)(8676002)(2906002)(83380400001)(110136005)(33656002)(9686003)(316002)(6506007)(71200400001)(53546011)(54906003)(55016002)(7416002)(107886003)(52536014)(86362001)(8936002)(478600001)(26005)(5660300002)(4326008)(64756008)(66946007)(76116006)(66556008)(66476007)(921005)(66446008)(186003);
 DIR:OUT; SFP:1102; 
...
X-MS-Exchange-CrossTenant-originalarrivaltime: 01 Dec 2020 09:36:26.5470 (UTC)
...
Great suggestion, responses inline below. We'll remove build-time SoC depen=
dency driver code and submit an updated v2 patch.
...
> generations, but not at the cost of making the build of the driver SoC-sp=
ecific.
...
> MACH_ASPEED_G6)
> >  	help
> > -	  Support for error detection and correction on the Aspeed AST 2500 S=
oC.
> > +	  Support for error detection and correction on the Aspeed AST BMC So=
C.
> >
...
> > int val)  {
> >  	void __iomem *regs =3D (void __iomem *)context; @@ -53,7 +50,6 @@
> > static int regmap_reg_write(void *context, unsigned int reg, unsigned
> > int val)
> >  	return 0;
> >  }
...
> > int *val)  {
> >  	void __iomem *regs =3D (void __iomem *)context; @@ -63,6 +59,76 @@
> > static int regmap_reg_read(void *context, unsigned int reg, unsigned
> > int *val)
> >  	return 0;
> >  }
...
> > +
> > +static const u32 ast2400_dram_table[] =3D {
> > +	0x04000000,	//64MB
> > +	0x08000000,	//128MB
> > +	0x10000000,	//256MB
> > +	0x20000000,	//512MB
...
> > +
> > +static const u32 ast2500_dram_table[] =3D {
> > +	0x08000000,	//128MB
> > +	0x10000000,	//256MB
> > +	0x20000000,	//512MB
> > +	0x40000000,	//1024MB
> > +};
> > +
> > +static const u32 ast2600_dram_table[] =3D {
> > +	0x10000000,	//256MB
> > +	0x20000000,	//512MB
> > +	0x40000000,	//1024MB
> > +	0x80000000,	//2048MB
...
> > +	size =3D ast2600_dram_table[reg04 & 0x3]; #elif
> > +defined(CONFIG_MACH_ASPEED_G5)
> > +	size =3D ast2500_dram_table[reg04 & 0x3]; #else
> > +	size =3D ast2400_dram_table[reg04 & 0x3]; #endif
> > +	return size;
> > +}
> > +EXPORT_SYMBOL(aspeed_get_dram_size);
>=20
> The driver must support running on any of the SoC generations without bei=
ng
> recompiled. This requires the driver be compiled for a specific SoC gener=
ation.
>=20
> The right way to do this is to use the OF match table data. Please fix it=
.
>=20
Fixed in v2, driver can check the DRAM controller hardware version from MCR=
_CONF [31:28].

> > +
> > +static const u32 aspeed_vga_table[] =3D {
> > +	0x800000,	//8MB
> > +	0x1000000,	//16MB
> > +	0x2000000,	//32MB
> > +	0x4000000,	//64MB
...
> > +	regmap_read(aspeed_regmap, ASPEED_MCR_CONF, &reg04);
> > +
> > +	size =3D aspeed_vga_table[((reg04 & 0xC) >> 2)];
> > +	return size;
> > +}
...
> > -	np =3D of_find_node_by_path("/memory");
> > +#ifdef CONFIG_MACH_ASPEED_G4
> > +	np =3D of_find_node_by_path("/memory@40000000");
> > +#else
> > +	np =3D of_find_node_by_path("/memory@80000000");
> > +#endif
>=20
> Again, this makes the driver SoC-specific at compile time, and that's not=
 going
> to fly. If we need it, this should be solved with a phandle to the approp=
riate
> memory node.
>=20
...
The driver can handle both ECC on and off, so this check might not be neces=
sary.
...
> >
> > -
> >  static const struct of_device_id aspeed_of_match[] =3D {
> > +	{ .compatible =3D "aspeed,ast2400-sdram-edac" },
> >  	{ .compatible =3D "aspeed,ast2500-sdram-edac" },
...
> >
> > -
> >  static struct platform_driver aspeed_driver =3D {
> >  	.driver		=3D {
> >  		.name	=3D DRV_NAME,
