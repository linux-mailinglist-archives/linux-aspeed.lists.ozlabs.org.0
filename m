...
Received: from lists.ozlabs.org (lists.ozlabs.org [IPv6:2404:9400:2:0:216:3eff:fee1:b9f1])
	by mail.lfdr.de (Postfix) with ESMTPS id 1EC8A4B467B
	for <lists+linux-aspeed@lfdr.de>; Mon, 14 Feb 2022 10:43:46 +0100 (CET)
Received: from boromir.ozlabs.org (localhost [IPv6:::1])
	by lists.ozlabs.org (Postfix) with ESMTP id 4JxznC4ZLTz3cFh
	for <lists+linux-aspeed@lfdr.de>; Mon, 14 Feb 2022 20:43:43 +1100 (AEDT)
...
Received: from mx0a-001b2d01.pphosted.com (mx0b-001b2d01.pphosted.com
 ...
 by lists.ozlabs.org (Postfix) with ESMTPS id 4Jxzmt58vhz3c9d
 for <linux-aspeed@lists.ozlabs.org>; Mon, 14 Feb 2022 20:43:26 +1100 (AEDT)
...
+}
+
+static void aspeed_spi_chip_enable(struct aspeed_spi_chip *chip, bool en=
able)
+{
...
+
+	if (enable)
+		reg |=3D aspeed_spi_chip_write_bit(chip);
+	else
+		reg &=3D ~aspeed_spi_chip_write_bit(chip);
...
+	const struct aspeed_spi_data *data =3D aspi->data;
+
+	chip->ctl =3D aspi->regs + data->ctl0 + chip->cs * 4;
+
...
+}
+
+static int aspeed_spi_init_devices(struct platform_device *pdev, struct =
aspeed_spi *aspi)
+{
...
+		}
+
+		chip =3D &aspi->chips[cs];
+		chip->aspi =3D aspi;
+		chip->cs =3D cs;
+
+		ret =3D aspeed_spi_chip_init(chip);
+		if (ret) {
+			of_node_put(np);
+			return ret;
+		}
...
+			chip->clk_freq =3D ASPEED_SPI_DEFAULT_FREQ;
+
+		aspi->num_cs++;
+	}
+
...
+		return -ENODEV;
+
+	ctlr =3D spi_alloc_master(dev, sizeof(*aspi));
+	if (!ctlr)
+		return -ENOMEM;
+
...
+	}
+
+	ret =3D aspeed_spi_init_devices(pdev, aspi);
+	if (ret)
+		goto disable_clk;
+
+	if (!aspi->num_cs) {
...
+
+	ctlr->mode_bits =3D SPI_RX_DUAL | SPI_TX_DUAL | aspi->data->mode_bits;
+	ctlr->bus_num =3D pdev->id;
+	ctlr->mem_ops =3D &aspeed_spi_mem_ops;
+	ctlr->num_chipselect =3D aspi->num_cs;
...
+	struct aspeed_spi *aspi =3D spi_controller_get_devdata(ctlr);
+
+	aspeed_spi_enable(aspi, false);
